...
/**
 * create_node - program that allocates memory and creates a new 'N-ary tree' node
 * with the specified string content
 *
 ...
 *
 * if memory allocation for the node or the string duplication fails, the function
 * cleans up any allocated memory and returns NULL
 *
...
/**
 * insert_at_beginning - program that inserts a node at the beginning of the children
 * list of a specified parent node in an 'N-ary tree'
 *
 * this function updates the parent node's children list by performing the following steps:
 *
 * 1. if the parent already has children, sets the new node's next pointer to point
 *    to the current first child of the parent
 * 2. updates the parent's children pointer to point to the new node, effectively inserting
 ...
 * @node: the new node to be inserted as the first child of the specified parent node
 *
 * Return: nothing (void)
 */
...
/**
 * nary_tree_insert - program that creates a new 'N-ary tree' node with specified
 * string content and inserts it under a given parent node
 *
 * this function combines the creation of a new node and its insertion into the tree by:
 *
 * 1. creating a new node with the specified string content using create_node().
 * 2. if a parent is specified, inserting the new node as the first child of the parent
 *    using insert_at_beginning()
 *
