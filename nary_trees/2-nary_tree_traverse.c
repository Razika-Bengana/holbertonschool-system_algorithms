 ...
 * if the current node's depth exceeds the known maximum, it updates the maximum
 * depth accordingly;
 ...
 * @action: a callback function to be executed on each node, which takes the node
 *          and its depth as arguments
 * @depth: the depth of the current node in the tree, starting from 0 for the root
...
void nary_tree_traverse_helper(nary_tree_t const *node, void (*action)(nary_tree_t const *node, size_t depth),
...
/**
 * nary_tree_traverse - program that initiates traversal of an 'N-ary tree' from the root,
 * executing a user-defined action on each node
 *
 * this function leverages 'nary_tree_traverse_helper' to recursively traverse the tree,
 * applying the provided action at each node and determining the tree's maximum depth;
 ...
 * @action: a pointer to a function that will be executed for each node encountered;
